...
continuum.multisource.iter.v1 = function(X.list, Y, lambda, gam, rankJ, rankA, maxiter = 1000, conv = 1e-6, center.X = TRUE, scale.X = TRUE, center.Y = TRUE, scale.Y = TRUE,  orthIndiv = TRUE ,I.initial = NULL,svdinit = FALSE){
  G = length(X.list)
  ...
  # center and scale X
  {
    ...
    for (g in 1:G){
      p[g] = nrow(X.list[[g]])
      if (center.X){
        centerValues.X[[g]] = apply(X.list[[g]], 1, mean, na.rm = T)
      }else{
        centerValues.X[[g]] = rep(0, p[g])
      }
      if (scale.X){
  ...
  # center and scale preprocess Y
  {
    ...
    if (scale.Y){
      scaleValues.Y = norm(matrix(Y), type = "f") #*sqrt(length(Y))
    }else{
      scaleValues.Y = 1
    }
    Y = sweep(matrix(Y), 2, centerValues.Y)
  ...
  if (is.null(I.initial)){
    # X.heter.list = lapply(1:G, function(g) matrix(0, nrow = p[g], ncol = p))
    X.heter.list = lapply(1:G, function(g) matrix(0, nrow = nrow(X.list[[g]]), ncol = n))
  }else{
    X.heter.list = I.initial
  }
  ...
  while (nrun < maxiter & !converged){
    ...
    if (rankJ){
      if (gam == 1e10){
        ml.homo = svd(t(X.homo), nu = rankJ, nv = rankJ)
        C = ml.homo$v
      }else{
        ml.homo = continuum.ridge.fix(t(X.homo), Y.homo, G, lambda = lambda, gam = gam, om = rankJ)
        C = ml.homo$C
      }
    }
    S = list.append(S, C)
    # U. = t(X.homo)%*%C%*%SOLVE(t(C)%*%C)
    # U = list.append(U, U.)
    ...
    X.homo.list = list()
    for (g in 1:G){
      X.heter.list[[g]] = (X.list[[g]] - X.homo[1:p[g],])%*%(diag(n)-t(X.homo)%*%SOLVE((X.homo)%*%t(X.homo))%*%X.homo)
      X.homo.list[[g]] = X.homo[1:p[g],]
      X.homo = X.homo[-(1:p[g]),]
    }
    X.homo = temp
    ...
    for (g in 1:G){
      if (rankA[g]){   # estimate
        ...
        if (orthIndiv & nrun>0){  # orthogonalization
          for (j in (1:G)[-g]){
            temp <- X.heter.list[[g]]%*%(diag(n)-t(X.heter.list[[j]])%*%SOLVE((X.heter.list[[j]])%*%t(X.heter.list[[j]]))%*%X.heter.list[[j]])
            # temp <- temp %*% (diag(n) - Cind[[j]] %*%
            #   t(Cind[[j]]))
          }
        }
        if (gam == 1e10){
          ml.heter = svd(t(temp), nu = rankA[g], nv = rankA[g])
          Cind[[g]] = ml.heter$v
        }else{
          ml.heter = continuum.ridge.fix(t(temp), Y.heter, 1, lambda = lambda, gam = gam, om = rankA[g])
          Cind[[g]] = ml.heter$C
        }
      }
    }

    XC.list = lapply(1:G, function(g) t(X.heter.list[[g]])%*%Cind[[g]])
    ct.heter = lapply(1:G, function(g)
      diag((t(XC.list[[g]])%*%XC.list[[g]])^(gam-1))*(t(XC.list[[g]])%*%Y.heter)^2)

    beta.Cind = lapply(1:G, function(g) C2beta(t(X.heter.list[[g]]), Y.heter, Cind[[g]], lambda)$beta)
    Yhat.heter.list = lapply(1:G, function(g) t(X.heter.list[[g]])%*%beta.Cind[[g]])
    Yhat.heter = do.call("+", Yhat.heter.list)
    ...
    if (orthIndiv & nrun == 0){
      for (g in 1:G){
        for (j in (1:G)[-g]) {
          ...
          # X.heter.list[[g]] <- X.heter.list[[g]]%*%(diag(n)-Cind[[g]] %*%
          #   t(Cind[[g]]))
        }
        if (rankA[g]){   # estimate
          if (gam == 1e10){
            ...
            ml.heter = continuum.ridge.fix(t(X.heter.list[[g]]), Y.heter, 1, lambda = lambda, gam = gam, om = rankA[g])
            Cind[[g]] = ml.heter$C
          }
        }
      }
    ...
    if (gam == 1e10){
      # if (!nrun%%10){
        # print(norm(R, type = "f"))
        # print(c(norm(X.homo - X.homo.last, type = "f"),norm(do.call(rbind, X.heter.list)-
        #   X.heter.last, type = "f")))
      # }
      # if (norm(Rlast - R, type = "f") <= conv){
      #   converged <- T
  ...
  for (g in 1:G){
    temp = temp+p[g]
    beta.C[(temp-p[g]+1):temp] = beta.C[(temp-p[g]+1):temp]/scaleValues.X[[g]]*scaleValues.Y
    beta.Cind[[g]] = beta.Cind0[[g]]/scaleValues.X[[g]]*scaleValues.Y
    intercept = intercept - t(beta.C[(temp-p[g]+1):temp])%*%centerValues.X[[g]] - t(beta.Cind[[g]])%*%centerValues.X[[g]]
  }
  ...
  return(list(C = C, Cind = Cind, beta.C = beta.C, beta.Cind = beta.Cind,
              ...
              scaleValues.X = scaleValues.X, scaleValues.Y = scaleValues.Y,
              intercept = intercept,
              S = S, T = Sind, U = U, converged = converged,
              J = X.homo, I = X.heter.list, nrun = nrun))
}
